...
describe('dependency resolution', function () {
  ...
  describe('class Resolver', function () {
    ...
    describe('cache behavior', function () {
      ...
      beforeEach(function () {
        fixturePath = resolveFixturePath('test-with-transitive-dep.fixture.js');
        transitiveDepFixturePath = resolveFixturePath(
          'transitive-dep.fixture.js'
        );
        depFixturePath = resolveFixturePath('dependency.fixture.js');
        resolver.resolveDependencies(fixturePath);
      });

      it('should maintain a cache of "seen" paths', function () {
        expect(
          resolver._cache,
          'to have keys',
          fixturePath,
          transitiveDepFixturePath,
          depFixturePath
        );
      });

      describe('when re-run against same path', function () {
        let cache;
        ...
        it('should return the resolved modules in its cache', function () {
          expect(resolver._cache, 'to satisfy', {
            [fixturePath]: expect.it('to be', cache[fixturePath]),
            [depFixturePath]: expect.it('to be', cache[depFixturePath]),
            [transitiveDepFixturePath]: expect.it(
              'to be',
              cache[transitiveDepFixturePath]
            ),
          });
        });

        describe('when `refreshPaths` option provided', function () {
          beforeEach(function () {
            cache = {...resolver._cache};
            resolver.resolveDependencies(fixturePath, {
              refreshPaths: [fixturePath],
            });
          });

          it('should resolve the modules again', function () {
            expect(resolver._cache, 'to satisfy', {
              [fixturePath]: expect.it('not to be', cache[fixturePath]),
              [depFixturePath]: expect.it('to be', cache[depFixturePath]),
              [transitiveDepFixturePath]: expect.it(
                'to be',
                cache[transitiveDepFixturePath]
        ...
        describe('when `force` option provided', function () {
          beforeEach(function () {
            cache = {...resolver._cache};
            resolver.resolveDependencies(fixturePath, {
              force: true,
          ...
          it('should resolve the modules again', function () {
            expect(resolver._cache, 'to satisfy', {
              [fixturePath]: expect.it('not to be', cache[fixturePath]),
              [depFixturePath]: expect.it('not to be', cache[depFixturePath]),
              [transitiveDepFixturePath]: expect.it(
                'not to be',
                cache[transitiveDepFixturePath]
              ),
        ...
        describe('when both `force` and `refreshPaths` options provided', function () {
          beforeEach(function () {
            cache = {...resolver._cache};
            resolver.resolveDependencies(fixturePath, {
              force: true,
          ...
          it('should resolve all the modules again, overriding `refreshPaths`', function () {
            expect(resolver._cache, 'to satisfy', {
              [fixturePath]: expect.it('not to be', cache[fixturePath]),
              [depFixturePath]: expect.it('not to be', cache[depFixturePath]),
              [transitiveDepFixturePath]: expect.it(
  ...
  describe('resolveDependencies() convenience function', function () {
      ...
      describe('when not provided a path to TS config file', function () {
        ...
        describe('when TS config file not in `cwd`', function () {
          ...
          beforeEach(function () {
            ...
            result = resolveDependencies(fixture, {cwd: path.dirname(fixture)});
          });
    ...
    describe('when provided a JavaScript file', function () {
      ...
      describe('when not provided a path to a Webpack config file', function () {
        /**
         * @type {import('../../src/resolver').ResolvedDependencyInfo}
         */
        let result;
        let fixture;
